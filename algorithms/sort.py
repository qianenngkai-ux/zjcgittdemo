...
- 要求元素之间可排序，即支持完整的大小比较（`<` `<=` `>` `>=`）。
  数字与字符串天然满足；自定义类型只要用 `functools.total_ordering`
  装饰、实现 `__eq__` 与 `__lt__`，其余运算符会自动补全。
  注意这里比内置 `sorted()` 的要求更严：`sorted()` 只需要 `__lt__`，
  而本模块的六个算法用到的运算符各不相同（见下表），只定义部分
  运算符会抛 `TypeError`。
- 稳定与否，各自在下方的 docstring 中说明

各算法实际用到的比较运算符（测试 `test_sorts_custom_comparable_type`
盯住的就是这条契约）：
...
def selection_sort(items):
    ...
    注意它的代价：无论输入是否有序，比较次数恒为 n(n−1)/2，**没有
    提前退出机制**。因此对已排序输入，它比同样带 O(n²) 最坏界的
    冒泡排序和插入排序慢约三个数量级 —— 实测 20000 个已排序元素，
    冒泡 0.5ms、插入 1.0ms，而选择排序 3389ms。基本有序的数据应当
    选插入排序。
