...
135 个用例覆盖空列表、单元素、已排序、逆序、重复值、负数、浮点数，以及「不修改原列表」「返回值不是入参别名」等约定。

测试分两层，靠混入复用，新增算法不必重写测试：

- **`SortContractMixin`（21 项）** —— 六个算法都继承。每个算法只需声明自己的 `sort` 函数即可自动获得全部边界检查。
- **`StableSortMixin`（3 项）** —— 只被声明为「稳定」的三个算法继承（bubble / insertion / merge），断言相等元素保持原有相对顺序。另外三个声明为非稳定，不继承 —— 非稳定意味着「不保证」而非「一定打乱」，对它断言顺序变化属于依赖实现的测试。

另外，测试使用了一个自定义可比较类型 `Ranked`（只实现 `__eq__` 与 `__lt__`，其余由 `functools.total_ordering` 补全），用来验证「元素可排序」这条契约不局限于数字和字符串。只用 int/float/str 做测试的话，某个算法多依赖一个比较运算符是不会被发现的。

其中包含针对快排最坏情况的回归用例：2000 个元素的已排序、逆序、全相同输入，对**全部六个算法**运行。这三条曾经会让程序因递归过深而崩溃。
